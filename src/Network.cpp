...
Network::Network(const Network & parent1, const Network & parent2) : Network() {
  if (parent1.getNumberOfInputs() != parent2.getNumberOfInputs()) {
    throw std::invalid_argument(
      "Numbers of inputs of the parents are not equal."
    );
  }
  if (parent1.getNumberOfOutputs() != parent2.getNumberOfOutputs()) {
    throw std::invalid_argument(
      "Number of outputs of the parents are not equal."
    );
  }
  unsigned int numberOfInputs = parent1.getNumberOfInputs();
  unsigned int numberOfOutputs = parent1.getNumberOfOutputs();
  unsigned int numberOfNoninputs1 = parent1.getNumberOfNeurons() - numberOfInputs;
  unsigned int numberOfNoninputs2 = parent2.getNumberOfNeurons() - numberOfInputs;
 
  unsigned int numberOfNeurons = numberOfInputs
                               + numberOfNoninputs1
                               + numberOfNoninputs2
                               + numberOfOutputs;
  for(unsigned int i=0; i<numberOfNeurons; i++) {
    ...
    if ( i < numberOfInputs ) {
      ...
      if (i < numberOfInputs + numberOfNoninputs1) {
        getNeuronByIndex(i)->setBias(parent1.getNeuronByIndex(i)->getBias());
      }
      else if (i < numberOfNeurons - numberOfOutputs) {
        getNeuronByIndex(i)->setBias(parent2.getNeuronByIndex(i - numberOfNoninputs1)->getBias());
      }
      else {
        addOutput(getNeuronByIndex(i));
      }
  ...
  for(unsigned int i=0; i<parent1.getNumberOfNeurons(); i++) {
    Neuron* neuron = parent1.getNeuronByIndex(i);
    for(unsigned int j=0; j<neuron->getNumberOfSources(); j++) {
      connect(parent1.getIndexByNeuron(neuron->getSource(j)), i, neuron->getWeight(j));
    }
  }

  for(unsigned int i=0; i<parent2.getNumberOfNeurons(); i++) {
    Neuron* neuron = parent2.getNeuronByIndex(i);
    for(unsigned int j=0; j<neuron->getNumberOfSources(); j++) {
      unsigned int sourceindex = parent2.getIndexByNeuron(neuron->getSource(j));
      std::cout << "sourceindex = " << sourceindex << '\n';
      if (sourceindex >= numberOfInputs) {
        sourceindex += numberOfNoninputs1;
      }
      std::cout << "sourceindex = " << sourceindex << '\n';
      connect(sourceindex, i + numberOfNoninputs1, neuron->getWeight(j));
    }
  }

  for(unsigned int i=0; i<numberOfOutputs; i++) {
    for(unsigned int j=0; j<numberOfOutputs; j++) {
      unsigned int sourceindex1 = numberOfInputs
                                + numberOfNoninputs1
                                - numberOfOutputs
                                + i;
      unsigned int sourceindex2 = numberOfInputs
                                + numberOfNoninputs1
                                + numberOfNoninputs2
                                - numberOfOutputs
                                + i;
      unsigned int targetindex  = numberOfNeurons
                                - numberOfOutputs
                                + j;
      connect(sourceindex1, targetindex, 0.0);
      connect(sourceindex2, targetindex, 0.0);
    }
  }
}
